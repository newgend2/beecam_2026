...
DATA_LABEL="DATA"
FSTAB="/etc/fstab"
MARKER="${MOUNTPOINT}/.weather-station-data-initialized"
SEED_CONFIG_DIRS=(
...
if [[ ! -b "$DATA_PART" ]]; then
    ...
    if (( FREE_SECTORS < MIN_FREE_SECTORS )); then
        ...
        log "run partition_weather_station_sd_on_pc.sh from a Linux PC/laptop"
        exit 2
    fi
...
mkdir -p "${MOUNTPOINT}/logs" "${MOUNTPOINT}/weather"

if [[ ! -e "${MOUNTPOINT}/configs" ]]; then
    for seed_dir in "${SEED_CONFIG_DIRS[@]}"; do
        if [[ -d "$seed_dir" ]]; then
            log "copying default configs from ${seed_dir} to ${MOUNTPOINT}/configs"
            mkdir -p "${MOUNTPOINT}/configs"
            cp -a "${seed_dir}/." "${MOUNTPOINT}/configs/"
            break
        fi
    done
fi
...
if [[ ! -e "${MOUNTPOINT}/configs" ]]; then
    log "no default config directory was found; /data/configs was not created"
fi
